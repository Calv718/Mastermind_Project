...
It build off the implementation for deadline 3 with:
 - dynamically changing the generationg of guesses depending on which SCSA is being used.
    - Mystery SCSA are still using enumeration for now while we conttinue trying to optimize for the partially observable SCSA's
    - InsertColors generator is failing 
 - reducing the search space with filters specific to each SCSA
 - using the feedback to determine if the guess is consistent with feedback given
 - has a simple heursitic to minimize the solution space
 - Uses a time buffer in order to keep guesses under 5 seconds
    - Need to implement mor efficient code generation for InsertColors, PreferFewer, and UsuallyFewer
"""
...
class LogicLegends(Player):
    ...
    def generate_InsertColors(self, length: int, colors: list[str]) -> list[str]:
        ...
        # # Divides the total number of combinations by 1000, to later reduce search space by scale of 1000
        # num_to_select = len(all_combinations) // 1000

        # # Returns a sample of num_to_select random unique elements from all combos
    ...
    def generate_Mystery1(self, length: int, colors: list[str]) -> list[str]:
        """
        A placeholder generator used to generate the possible codes for mystery1 SCSA
        """
        # Uses a list comprehension for efficient generation of all possible combinations
        return [''.join(p) for p in product(colors, repeat=length)]
    ...
    def generate_Mystery2(self, length: int, colors: list[str]) -> list[str]:
        """
        A placeholder generator used to generate the possible codes for mystery2 SCSA
        """
        # Uses a list comprehension for efficient generation of all possible combinations
        return [''.join(p) for p in product(colors, repeat=length)]
    ...
    def generate_Mystery3(self, length: int, colors: list[str]) -> list[str]:
        """
        A placeholder generator used to generate the possible codes for mystery3 SCSA
        """
        # Uses a list comprehension for efficient generation of all possible combinations
        return [''.join(p) for p in product(colors, repeat=length)]
    ...
    def generate_Mystery4(self, length: int, colors: list[str]) -> list[str]:
        """
        A placeholder generator used to generate the possible codes for mystery4 SCSA
        """
        # Uses a list comprehension for efficient generation of all possible combinations
        return [''.join(p) for p in product(colors, repeat=length)]
    ...
    def generate_Mystery5(self, length: int, colors: list[str]) -> list[str]:
        """
        A placeholder generator used to generate the possible codes for mystery5 SCSA
        """
        # Uses a list comprehension for efficient generation of all possible combinations
        return [''.join(p) for p in product(colors, repeat=length)]
